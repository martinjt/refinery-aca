...
export RESOURCE_GROUP="honeycomb-refinery$SUFFIX"
export ENVIRONMENT_NAME="refinery-env$SUFFIX"
export LOCATION=uksouth
export STORAGE_ACCOUNT_NAME="refineryappstorage$SUFFIX"
export STORAGE_SHARE_NAME="refinery-config"
export STORAGE_MOUNT_NAME="refinerymount"
export CONTAINER_APP_NAME="refinery"
export IMAGE_NAME=honeycombio/refinery:1.16.0
export HONEYCOMB_API_KEY=$1

echo "Creating a Refinery instance in an Azure Container App"
...
az containerapp create \
  ...
  --env-vars "REFINERY_HONEYCOMB_API_KEY=secretref:honeycomb-api-key" "REFINERY_QUERY_AUTH_TOKEN=refinery$SUFFIX" \
  --output none 2>/dev/null
...
yq -i '
  ...
  .properties.template.containers[0].volumeMounts[0].mountPath = "/etc/refinery" |
...
echo ""
echo "======"
echo "Refinery is at $CONTAINER_APP_INGRESS"
...
echo "   --name \"Refinery In Azure Container Apps\" \ "
echo "   --start \$(date +%s.%N) \ "
echo "   --end \$(date +%s.%N) \ "
echo "   --verbose"
echo ""
